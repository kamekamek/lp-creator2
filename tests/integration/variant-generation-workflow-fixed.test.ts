/**
 * LP Creator Platform - Variant Generation Workflow Integration Tests (Fixed)
 */
...
describe('Variant Generation Workflow Integration', () => {
  ...
  test('should validate tool structure and parameters', () => {
  ...
  test('should handle mock data correctly in test environment', async () => {
    ...
    const mockResult: VariantGenerationResult = {
      success: true,
      variants: [
        {
  ...
  test('should validate parameter constraints', () => {
    ...
    // Test variant count constraints
    expect(() => params.shape.variantCount.parse(1)).not.toThrow();
    expect(() => params.shape.variantCount.parse(3)).not.toThrow();
    expect(() => params.shape.variantCount.parse(0)).toThrow();
    expect(() => params.shape.variantCount.parse(4)).toThrow();
    
    // Test focus areas constraints
    expect(() => params.shape.focusAreas.parse(['modern-clean'])).not.toThrow();
    expect(() => params.shape.focusAreas.parse(['invalid-focus'])).toThrow();
  });

  // Integration tests that require actual AI calls
