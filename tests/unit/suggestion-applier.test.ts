...
import { SuggestionApplier } from '../../src/utils/suggestion-applier';
...
describe('SuggestionApplier', () => {
  ...
  describe('applySuggestionToContent', () => {
    it('H1タグ追加提案を正しく適用できる', () => {
      ...
      const result = SuggestionApplier.applySuggestionToContent(sampleHtml, sampleCss, suggestion);
    ...
    it('ボタンテキスト強化提案を正しく適用できる', () => {
      ...
      const result = SuggestionApplier.applySuggestionToContent(originalHtml, sampleCss, suggestion);
  ...
  describe('エラーハンドリング', () => {
    it('不正なアクションタイプに対してエラーを投げない', () => {
      ...
      expect(() => {
        SuggestionApplier.applySuggestionToContent(sampleHtml, sampleCss, invalidSuggestion);
      }).not.toThrow();
    });
    ...
    it('空のHTMLに対しても正常に動作する', () => {
      ...
      expect(() => {
        SuggestionApplier.applySuggestionToContent('', '', suggestion);
      }).not.toThrow();
    });
  });
  ...
  describe('重複適用の防止', () => {
    it('既にH1タグがある場合、追加のH1を作成しない', () => {
      ...
      const result = SuggestionApplier.applySuggestionToContent(htmlWithH1, sampleCss, suggestion);
    ...
    it('既にmeta descriptionがある場合、追加しない', () => {
      ...
      const result = SuggestionApplier.applySuggestionToContent(htmlWithMeta, sampleCss, suggestion);
