...
/**
 * 提案適用ユーティリティ
 * AISuggestionを実際のHTMLコンテンツに適用する
 */
export class SuggestionApplier {
      ...
      switch (suggestion.action.type) {
        case 'add':
          ({ htmlContent: updatedHtml, cssContent: updatedCss } = 
            this.handleAddAction(document, cssContent, suggestion));
          break;
          
        case 'modify':
          ({ htmlContent: updatedHtml, cssContent: updatedCss } = 
            this.handleModifyAction(document, cssContent, suggestion));
          break;
          
        case 'replace':
          ({ htmlContent: updatedHtml, cssContent: updatedCss } = 
            this.handleReplaceAction(document, cssContent, suggestion));
          break;
  ...
  private static addH1Tag(document: Document, suggestion: AISuggestion): void {
    ...
    if (existingH1) return; // 既にH1がある場合は追加しない
