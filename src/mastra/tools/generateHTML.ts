...
function generateAccessibilityElements() {
  return {
    ariaLabels: [
    ...
    skipLinks: '<a href="#main" class="skip-link">メインコンテンツへスキップ</a>',
  };
}
...
function generateProblemSection() {
  return `
        <!-- 問題提起セクション -->
...
function generateSolutionSection() {
  return `
        <!-- 解決策セクション -->
            ...
            <div class="container">
                ...
                <div class="solution-grid">
                    <div class="solution-card">
                        <i class="bi bi-graph-up-arrow solution-icon" aria-hidden="true"></i>
                        <h3>コンバージョン率3倍UP</h3>
                        <p>心理学に基づいたコピーライティングとUXデザインで、確実に成果を向上させます</p>
                    </div>
                    <div class="solution-card">
                        <i class="bi bi-lightning-charge solution-icon" aria-hidden="true"></i>
                        <h3>最短1週間でリリース</h3>
                        <p>豊富なテンプレートと効率的な制作フローで、スピーディーな納品を実現</p>
                    </div>
                    <div class="solution-card">
                        <i class="bi bi-currency-dollar solution-icon" aria-hidden="true"></i>
                        <h3>コスパ最強の価格設定</h3>
                        <p>高品質でありながら、中小企業でも導入しやすい価格でサービスを提供</p>
                    </div>
                </div>
            </div>
        </section>`;
  ...
  return `
        ...
        <section id="contact" class="contact" role="region" aria-labelledby="contact-title">
            <div class="container">
                ...
                <form class="contact-form" aria-label="お問い合わせフォーム" method="POST" action="/api/contact">
                    <!-- CSRF Protection -->
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" class="form-input" required 
                               aria-describedby="name-help" maxlength="100" autocomplete="name">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="email" name="email" class="form-input" required 
                               aria-describedby="email-help" maxlength="255" autocomplete="email">
                        <div id="email-help" class="form-help">連絡可能なメールアドレスをご入力ください</div>
                    </div>
                    <div class="form-group">
                        <label for="company" class="form-label">会社名</label>
                        <input type="text" id="company" name="company" class="form-input" maxlength="100" autocomplete="organization">
                    </div>
                    <div class="form-group">
                        ...
                        <textarea id="message" name="message" class="form-textarea" rows="5" required 
                                  aria-describedby="message-help" maxlength="1000"></textarea>
...
export const generateHTMLTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    const accessibility = generateAccessibilityElements();
    
    // HEAD要素とBODY要素の生成
    const head = generateHeadSection(structuredData);
    const body = generateBodySection();
...
${body}
    ...
    return {
      ...
      accessibility: {
        ...
        landmarks: accessibility.landmarks,
